...
get_latest_tag_via_git() {
  # Fallback: use git ls-remote to read tags without GitHub API.
  # This only tells us the newest tag name; it does NOT create a release.
  # If there is no GitHub Release asset for that tag, installation will fail.
  local repo="https://github.com/${REPO_OWNER}/${REPO_NAME}.git"
  # Extract tag names, keep only v* tags, sort by version, pick the highest
...
get_latest_tag() {
  ...
  tag="$(get_latest_tag_via_git || true)"
  if [[ -n "$tag" ]]; then
    echo "$tag"
    return 0
  fi
...
install_core_from_release() {
  ...
  tag="$(get_latest_tag || true)"
  if [[ -z "$tag" ]]; then
    err "Could not detect latest tag (release redirect blocked and git fallback failed)."
  ...
  if ! download_with_prefixes "$url" "$tgz"; then
    rm -rf "$tmpd" || true
    err "Download failed. Either connection/mirror is blocked, or the GitHub Release asset for tag '${tag}' does not exist yet."
    err "URL: $url"
    err "Hint: push the tag and make sure GitHub Actions created a Release for it (or create the release manually)."
    return 1
  fi
